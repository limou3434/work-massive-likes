...
13. [高可用优化]
    ...
    这个问题可以后面再说...
    - 监控高可用:
         (1)硬件监控: 收集服务器 CPU 使用率、内存占用、磁盘 IO 状态、网络吞吐量等基础指标
         (2)中件监控: Redis 命中率与内存分片情况、Pulsar 队列堆积量、TiDB 集群健康状态与查询响应时间
         (3)应用监控: 接口响应时间、QPS、错误率、JVM 堆内存与 GC 状况、虚拟线程数量
         监控数据通过 Prometheus 采集, 配置多维度告警阈值, 重要指标变化时通过钉钉、企微、邮件等渠道实时通知运维人员

14. [灾备演练与容灾切换]
    根据"混沌工程"思想, 定期模拟各类故障场景, 比如:
    - 随机关闭服务组件
    - 模拟网络延迟分区
    - 注释错误数据和异常请求
    - 测量恢复时间(RTO)
