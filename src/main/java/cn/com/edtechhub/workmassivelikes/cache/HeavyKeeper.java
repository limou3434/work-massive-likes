...
public class HeavyKeeper implements TopK {
    ...
    public AddResult add(String key, int increment) {
        ...
        int maxCount = 0; // 用于追踪当前元素在所有桶中计数的最大值, 用于判断是否进入 TopK
        ...
        for (int i = 0; i < depth; i++) {
            ...
            synchronized (bucket) {
                ...
                // 如果此时桶不空, 并且发生冲突, 则衰减计数
                else {
                    for (int j = 0; j < increment; j++) {
                        double decay = bucket.count < LOOKUP_TABLE_SIZE ? // 根据桶中的当前计数值决定衰减系数
                                lookupTable[bucket.count] :
                                lookupTable[LOOKUP_TABLE_SIZE - 1];

                        if (random.nextDouble() < decay) { // 使用随机数决定是否执行衰减
                            bucket.count--;
        ...
        synchronized (minHeap) {
            ...
            // 如果当前 key 已经在最小堆里面, 则将其移出最小堆, 然后更新计数重新添加到最小堆中
            if (existing.isPresent()) {
                minHeap.remove(existing.get());
                minHeap.add(new Node(key, maxCount));
                isHot = true;
            }
            // 如果当前 key 不在最小堆里面, 则检查最小堆是否已经满了
            else {
                // 如果最小堆未满, 或者当前元素的计数大于等于最小堆中最小的元素的计数(避免一堆低频元素不停污染 TopK 列表), 则将当前元素添加到最小堆中
                if (minHeap.size() < k || maxCount >= Objects.requireNonNull(minHeap.peek()).count) {
                    Node newNode = new Node(key, maxCount);
                    // 如果最小堆已经满了, 则将最小堆中的最小元素移出最小堆, 并将其添加到被挤出的元素阻塞队列中
                    if (minHeap.size() >= k) {
